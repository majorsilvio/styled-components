...
const getRGBcolor = ({ clr, progress, mode }: ProgressProps) => {
    ...
    return `linear-gradient(${mode === 'left' ? 'to left' : 'to right'},rgb(${r} ${g} ${b} / 0.3) 0% ,${mode === 'middle' ? `rgb(${r} ${g} ${b} / ${progress / 100 + 0.3}) 50%,rgb(${r} ${g} ${b} / 0.3) 100%)` : `rgb(${r} ${g} ${b} / ${progress / 100 + 0.3}) 100%)`}`
}
...
const before = ({progress,showProgress}: ProgressProps) => css`
    &:before{
        content: '';
        text-align: center;
